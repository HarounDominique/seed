...
- Detect the dominant language(s) from file extensions actually present (not from a
  README claim) — this sets `commit-guard.sh`'s `PRODUCTION_EXTS` default relevance and
  the test-naming convention already in use (`test_*.py` vs `*.test.js` vs `*_test.go`),
  which may differ from `scripts/commit-guard.sh`'s stock `TEST_NAME_PATTERNS`.
...
Run `${CLAUDE_PLUGIN_ROOT}/scripts/init-state-db.sh` to create the hot-cache schema
(`spec_index`, `phase_state`, `citations`, `learned_rules`). **`${CLAUDE_PLUGIN_ROOT}` is
...
shell has nothing to expand it to. The script prefers a `sqlite3` CLI and falls back to
`python3`/`python`'s stdlib `sqlite3` module automatically — report a hot-cache creation
...
plugin. Copy `${CLAUDE_PLUGIN_ROOT}/scripts/commit-guard.sh` to `scripts/commit-guard.sh`
in the project (create `scripts/` if absent), **then explicitly run
`chmod +x scripts/commit-guard.sh`** — do not assume the copy preserved the executable
bit; it frequently doesn't (a plain file write/copy across filesystems or tools often
drops it), and `/seed:doctor` check 5 exists specifically to catch an install that
skipped this. Its defaults detect
...
`scripts/commit-guard.sh` already exists (re-running init), leave it untouched — it may
...
`scripts/commit-guard.sh`) is the cold-truth baseline every later phase branches from —
...
1. `git add memory-bank/ .gitignore scripts/commit-guard.sh` (and any other file Steps
   2-4 touched).
